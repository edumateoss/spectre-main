% =============================================================================
% Autorizacion para deposito en biblioteca (plantilla oficial ETSIIT UGR).
% Sustituye DNI/fecha cuando lo tengas finalizado para la firma.
% =============================================================================

\thispagestyle{empty}

\vspace*{3cm}
\noindent Yo, \textbf{\tfgautor}, alumno de la titulacion \textbf{\tfgtitulacion} de la Escuela Tecnica Superior de Ingenierias Informatica y de Telecomunicacion de la \textbf{Universidad de Granada}, con DNI 77994113D, autorizo la ubicacion de la siguiente copia de mi Trabajo Fin de Grado en la biblioteca del centro para que pueda ser consultada por las personas que lo deseen.

\vspace{2.5cm}
\noindent Fdo: \tfgautor

\vspace{2.5cm}
\noindent Granada a X de \tfgmes\ de \tfganio.

\cleardoublepage
